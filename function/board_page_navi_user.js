...
function page_tag(total, row, col, now){
	...
	if(total>row){
		...
		txt +="<ul>";
			if(!(now_col_index<=1)){
				txt +="<li class='paging_img'> <a href='#'  onclick='goPage(" + (parseInt(now)-1) +")'><img src='../../images/btn_prev.gif'alt='' /></a></li>";
			}
			...
			if(total > now_col_index_end*row){
				txt +="<li class='paging_img'> <a href='#'  onclick='goPage(" + (parseInt(now)+1) +")'><img src='../../images/btn_next.gif'alt='' /></a></li>";
			}
		txt +="</ul>";
		
	}else{
		txt += "<ul>";
		txt += "	<li class='paging_img'><a href='#'><i class='fa fa-angle-left'></i></a></li>";
		txt += "	<li class='paging_on'><a href='#'>1</a></li>";
		txt += "	<li class='paging_img'><a href='#'><i class='fa fa-angle-right'></i></a></li>";
		txt += "</ul>";
	}
	return txt;
}
...
exports.action = function(brd_id, whereText, now_page, col_count, row_count, callback){
	...
	brd_mngtData(brd_id, whereText, function(error, result){
		if (error) 
			...
			callback(null,page_tag(result[0].total_cnt, row_count, col_cnt, now_page), page_str(now_page,row_count,col_cnt), row_count);
	});
}
