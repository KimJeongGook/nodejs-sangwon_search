...
exports.listener=function(request, response){
	...
	var main_event_y = request.param("main_event_y") == undefined ? "" : request.param("main_event_y");
	var main_event_n = request.param("main_event_n") == undefined ? "" : request.param("main_event_n");
	...
	if(mode=="write"){
		sql = "insert into "+table+"(" +
				"board_id, board_title, board_content, main_event_y, main_event_n, sort, orgmem_no, " +
				"strt_dt, end_dt, homepage,topcompany, create_id, create_date, create_host) " +
				"values('" + brd_id + 
				"', '" + brd_title + 
				"', '" + brd_content + 
				"', '" + main_event_y +
		...
		sql = "update "+table+" set board_id='" + brd_id +
			...
			"', main_event_y = '" + main_event_y +
			"', main_event_n = '" + main_event_n + 
			"', sort = '" + sort + 
			"', orgmem_no = '" + orgmem_no + 
			"', strt_dt = '" + strt_dt + 
			"', end_dt = '" + end_dt + 
		...
		async.series({
			 first : function(callback){
				db.query(sql, function(error, results){
					if(error){
						response.send("fail");
						console.log("admin/web/hongbo_board_save/error=====", sql);
					}else{
						if(mode == "write"){
							...
							db.query(select_sql, function(error, brd_no){
								if(error){
									...
									if(brdPushYn_result == "Y"){
										//var push_sql = "select key_num, type, popup_yn from push where phone_num != ? "; 
								    	...
								    	push_sql += ") as a left outer join push b on a.orgmem_phone = replace(b.phone_num, '-', '') and b.popup_yn = 'y'";
								    	console.log("push_sql ::::::" + push_sql);
											...
											if(results2.length > 0){
												...
												for(var i = 0; i < results2.length; i++){
													if(results2[i].type == "android"){
														console.log("results2[i].type == " + results2[i].type + " >> 안드로이드 >> " + results2[i].orgmem_phone);
														...
														ios_push_url = "/hongbo_board_view?brd_no=" + brd_no[0].brd_no;
														...
														if(smsYn_result == "Y") { // 문자알림이 설정되어 있으면
															...
															if(results2[i].orgmem_phone == '01045781783') {
		...
		if(filesArray.length > 0) {
			...
			for(var i = 0; i < filesArray.length; i++) {
				...
				db.query(file_sql, function(error, results){
					if(error){
						console.log("admin/web/hongbo_board_save/file/error===", file_sql);
						response.send("fail");
					} 
				});
			} 
